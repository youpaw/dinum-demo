...
Nix microVM (cloud-hypervisor), plus N Bureautix client VMs, and one
pre-seeded shared document for live co-editing.
...
| `nix/services.nix` | Single source of truth: guest address/tap/MAC, and each service's domain, manage command and proxy prefix |
| `nix/apps.nix` | Every app in one config: renders `nix/services.nix` into the shell data each script gets, declares each script's runtime tools, and generates the host `Caddyfile` + systemd unit installed by `host-install` |
| `nix/guest.nix` | The microVM: hardware, base OS, and the only place demo data meets the `demo.*` options |
| `nix/net.nix` | `demo.net.*` options — static IP/MAC → systemd-networkd + `/etc/hosts`, and the derived `hosts`/`origins` lists every service reuses |
...
```
   browsers ──► host LAN IP :80 ──► Caddy ──┬── /drive/*  (Host: drive.selfhostix)
                                            └── /         (Host: unchanged)
                                                   │
                      host 192.168.100.1 ─── tap-selfhostix ─── guest 192.168.100.10
                                                                 nginx :80
                                                                 ├── docs.selfhostix   (default server)
                                                                 └── drive.selfhostix
                                                                 dex :8080 · garage :9000
                                                                 postgres · redis ×2
```
...
gunicorn socket, own redis server, own database, own vhost). The host proxy
tells them apart by name: prefixed routes get `header_up Host <domain>`, and
Docs' vhost is the guest's default server, so bare-IP and LAN `Host` headers
land there.
...
sudo nix run .#host-install    # install + enable the host proxy unit
...
guest, propagate demo data. `nix run .#microvm -- eval` evaluates the guest
without booting it.
...
default the guest IP), `SELFHOSTIX_IP=...` (IP clients use for the service
domains, default: guest IP), `SELFHOSTIX_PUBLIC_ORIGIN=http://<host-LAN-IP>`
(browser origin trusted by Django; auto-detected at boot, override only if
detection picks the wrong uplink), `ALICE_PASS`/`BOB_PASS`, `SEED_DOC_TITLE`,
`SELFHOSTIX_URL` (seed link base, defaults to the detected origin),
...
lands on Docs via the proxy (`/` → Docs, `/drive/*` → Drive). Log in at
`/admin/` with a demo user (password login, no SSO roundtrip: the mock OIDC
issuer lives on the tap link and is unreachable from outside).
...
The backends keep `ModelBackend` enabled alongside OIDC, so a Django-admin
...
> unsafe request. Trusted origins are every service domain, the guest IP,
> localhost, plus the auto-detected host LAN origin
> (`SELFHOSTIX_PUBLIC_ORIGIN`) — all derived once in `nix/net.nix`
> (`demo.net.origins`, with `demo.net.hosts` for `DJANGO_ALLOWED_HOSTS`). If
> the host IP changes, reboot the guest so the new origin is picked up. After
> a rebuild, reset the password via `nix run .#users -- set-password` (or wipe
> `data/` for a fresh start).
>
